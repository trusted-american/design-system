...
const FormTextarea: TOC<FormTextareaSignature> = <template>
  ...
  <textarea
    id={{@identifier}}
    value={{@value}}
    class="block w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-blue-300 transition-all duration-300 ease-in-out {{if @size (concat 'form-control-' @size)}}"
    required={{@isRequired}}
